...
def buildSim(cppFlags, dir, type, pgo=None):
    ...
    DRAMPOWERPATH = os.environ["DRAMPOWERPATH"]
    BOOKSIMPATH = os.environ["BOOKSIMPATH"]

    env["CPPPATH"] = [xedPath,
            pinInclDir, joinpath(pinInclDir, "gen"),
            joinpath(PINPATH, "extras/components/include"), joinpath("/usr/include/hdf5/serial"), joinpath(DRAMPOWERPATH), joinpath(BOOKSIMPATH)]
    ...
    # DRAMPower
    env["PINLIBPATH"] += [DRAMPOWERPATH]
    # booksim
    env["PINLIBPATH"] += [BOOKSIMPATH]
    #External lib
    env["PINLIBS"] += ["boost_program_options", "booksim", "drampowerxml", "drampower",  "xerces-c", "icui18n", "icuuc", "icudata", "curl"]        
