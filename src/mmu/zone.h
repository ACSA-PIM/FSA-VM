...
struct FreeArea {
    // store start page description of 2^nr_free pages block , lru point to next
    // block
...
struct PerCpuPages {
    ...
    unsigned int
        batch; // number of pages need to be deleted from or added to page cache
...
class Zone {
  public:
    ...
    void set_pages_thres(uint64_t min, double low_rate = 1.25,
                         double high_rate = 1.5) {
        pages_min = min;
    ...
    void free_area_init_zone(MemoryNode *mem_node);
    /*#########------------------##########*/
    ...
    // two cpu page cache, one for hot page allocate, another for cold page
    // allocate
    std::vector<PerCpuPageset *> page_set;

    MemoryNode *node_ptr; // point to node which contained this zone
    void memmap_init_zone(MemoryNode *mem_node, Zone *zone, uint64_t start_pfn,
                          uint64_t end_pfn);
  ...
  public:
    ...
    Page *get_page_ptr(uint64_t page_id) {
        if (page_id >= node_page_num)
            std::cout << "page_id:" << page_id << " page_num:" << node_page_num
                      << std::endl;
