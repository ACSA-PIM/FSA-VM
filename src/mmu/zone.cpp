...
#include "zsim.h"
// init zone
Zone::Zone(ZoneType type, uint64_t start_pfn, uint64_t end_pfn)
    : zone_type(type), zone_start_pfn(start_pfn) {
...
void Zone::memmap_init_zone(MemoryNode *mem_node, Zone *zone,
                            uint64_t start_pfn, uint64_t end_pfn) {
    debug_printf("init zone memmap");
    free_area_init_zone(mem_node);
}

/*
 *@function: init free area
 */
void Zone::free_area_init_zone(MemoryNode *mem_node) {
    debug_printf("init free area");
    FlexiList<Page> *tmp_block_list =
        gm_memalign<FlexiList<Page>>(CACHE_LINE_BYTES, MAXORDER + 1);
    for (unsigned i = 0; i <= MAXORDER; i++) {
        free_area[i].block_list = new (&tmp_block_list[i]) FlexiList<Page>();
        free_area[i].nr_free = 0;
    }
    unsigned long max_block_num = 0;
    uint64_t page_index = zone_start_pfn;
    uint64_t page_number = free_pages;
    ...
    for (int i = MAXORDER; i > 0 && page_number > 0; i--) {
        ...
        if (max_block_num > 0) {
            ...
            for (unsigned long j = 0; j < max_block_num; j++) {
                free_area[i].block_list->push_block_back(
                    mem_node->get_page_ptr(page_index));
                page_index += (1 << i);
            }
        }
    }
    info("Maxorder list size: %d", free_area[MAXORDER].block_list->get_size());
    info("Free area init finished");
}
...
void Zone::zone_pageset_init(PerCpuPageset *ps) {
    ...
    if (zinfo->percpu_pagelist_fraction) {
        unsigned int high =
            (unsigned int)(free_pages / zinfo->percpu_pagelist_fraction);
...
MemoryNode::MemoryNode(unsigned id, Address start_addr = 0)
    : node_id(id), node_start_pfn(start_addr >> zinfo->page_shift),
      lowmem_kbytes(0), min_free_kbytes(0) {
    ...
    for (unsigned i = 1; i < MAX_NR_ZONES; i++) {
        ...
        zone_highest_possible[i] =
            Max(zinfo->max_zone_pfns[i], zone_lowest_possible[i]);
        debug_printf(
            "zone lowest possible %d:%lld , zone highest possible %d: %lld", i,
            zone_lowest_possible[i], i, zone_highest_possible[i]);
    }
    // calculate total page number of a node
    node_page_num = calculate_total_pages();
    present_pages = node_page_num;
    // allocate node_mem_map
    debug_printf("memory node: allocate node mem map , page num: %lld",
                 node_page_num);
...
void MemoryNode::init_zones() {
    ...
    for (unsigned i = 0; i < MAX_NR_ZONES; i++) {
        if (zone_highest_possible[i] != zone_lowest_possible[i]) {
            ...
            node_zones[i] = new (tmp_zone) Zone(
                (ZoneType)i, zone_lowest_possible[i], zone_highest_possible[i]);

            node_zones[i]->node_ptr = this;
            node_zones[i]->memmap_init_zone(this, node_zones[(ZoneType)i],
...
void MemoryNode::setup_per_zone_wmarks() {
    ...
    debug_printf("low memory size:%lld, min_free_kbytes:%lld", lowmem_kbytes,
                 min_free_kbytes);
    ...
    if (node_zones[Zone_HighMem]) {
        ...
        min_highmem =
            Min(Max(min_highmem, (uint64_t)SWAP_CLUSTER_MAX), (uint64_t)128);
