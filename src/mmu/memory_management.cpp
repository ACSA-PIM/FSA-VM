...
void BuddyAllocator::free_one_page(Zone *zone, uint64_t page_no,
                                   unsigned order) {
...
void BuddyAllocator::free_one_page(uint64_t page_no) {
    futex_lock(&buddy_lock);
    Page *page = mem_node->get_page_ptr(page_no);
    free_one_page(page->zone, page->pageNo, 0);
    futex_unlock(&buddy_lock);
}
 ...
 */
void BuddyAllocator::free_pcppages_bulk(Zone *zone, unsigned int count,
                                        PerCpuPages *pcp) {
    Page *page;
...
unsigned BuddyAllocator::allocate_bulk(Zone *zone, unsigned int order,
                                       uint64_t count,
                                       std::list<Page *> &list) {
    Page *page;
    uint64_t i = 0;
 ...
 *@param order:
 */
Page *BuddyAllocator::rmqueue_page_smallest(MemoryNode *mem_node, Zone *zone,
                                            unsigned order) {
    Page *page = NULL;
    for (unsigned current_order = order; current_order <= MAXORDER;
         current_order++) {
        ...
        page->set_page_node(mem_node);
        page->set_page_zone(zone);
 ...
 *we should adjust free_area,expand 2^high_order-2^low_order pages to
 *appropriate free area list, and this is what the function are doing
 *@param area:
 *@param low_order:
 *@param high_order:
 */
void BuddyAllocator::expand(MemoryNode *mem_node, Zone *zone, uint64_t page_id,
...
void BuddyAllocator::expand(MemoryNode *mem_node, Zone *zone,
                            uint64_t head_page_id, uint64_t page_id,
                            unsigned low_order, unsigned high_order) {
    FreeArea *area;
    Page *page;
    // get low page number
    ...
    while (low_order < high_order) {
        ...
        if ((page_id - head_page_id) < size) {
            // in first part
            new_page_id = head_page_id + size;
        } else {
            // in second part
            new_page_id = head_page_id;
            head_page_id = head_page_id + size;
        }
        // std::cout<<"new_page_id:"<<new_page_id<<std::endl;
        page = mem_node->get_page_ptr(new_page_id);
        // debug_printf("realloc page %lld,order: %d",new_page_id,high_order);
...
Page *BuddyAllocator::buffered_rmqueue(unsigned int gfp_mask, Zone *zone,
                                       unsigned order, unsigned cpu_id) {
    PerCpuPages *pps;
    Page *page = NULL;
    ...
    if (likely(order == 0)) {
        ...
        if (pps->page_list.empty()) {
            pps->count +=
                allocate_bulk(zone, order, pps->batch, pps->page_list);
        }
 ...
 *			 if cpu cache list has too much pages , free batch pages
 *to buddy system
 ...
 */
void BuddyAllocator::free_hot_cold_page(Page *page, unsigned cpu_id,
                                        bool cold) {
    // get zone
    Zone *zone = page->zone;
