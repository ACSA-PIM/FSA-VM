...
void debugSigHandler(int signum, siginfo_t* siginfo, void* dummy) {
    ...
    uint32_t debuggerPid = launchDebugger(callerPid, zsimAddrs);
...
static void printHeartbeat(GlobSimInfo* zinfo) {
    ...
    std::ofstream hb(std::string(zinfo->outputDir) + "/heartbeat");
...
void LaunchProcess(uint32_t procIdx) {
    ...
    if (cpid) { //parent
        ...
        info("Pin cmd for process %d: ", procIdx);
        for (unsigned int i = 0; i < args.size(); i++) {
            std::cout<<aptrs[i]<<" ";
        }
        std::cout<<std::endl;
...
void updateGMSize(uint32_t& gmSize, const char* procFile)
{
    if(procFile == nullptr) return;

    Config conf(procFile);
    uint32_t newSize;
    int i = 0;
    while (true)
    {
        ...
        if (!conf.exists(p_ss.str().c_str()))
            break;
        newSize = conf.get<uint32_t>(p_ss.str() + ".gmMBytes", 0);
        if (gmSize < newSize)
            gmSize = newSize;
        i++;
    }
...
int main(int argc, char *argv[]) {
    ...
    const char* procFile = nullptr;
    const char* profileFile = nullptr;
    if(argc > 2 && argc != 3 && argc != 4 && argc != 5){
        info("Usage %s config_file cmd_file output_dir profile_path", argv[0]);
        exit(1);
    }else {
        if (argc == 5){
            ...
            profileFile = realpath(argv[4], nullptr);
        }else if (argc == 4){
            procFile = realpath(argv[2], nullptr);
            outputDir = realpath(argv[3], nullptr);
            if( access(outputDir, F_OK) != 0){
    ...
    if(procFile == nullptr)
        ...
        Config proc_conf(procFile);
        pinCmd = new PinCmd(&conf, configFile, &proc_conf, procFile, outputDir, shmid,profileFile);
    }
    ...
    while (getNumChildren() > 0) {
        ...
        if (deadlockDetection) {
            if (simShouldAdvance) {
                ...
                if (numPhases <= lastNumPhases) {
                    ...
                    secsNotStalled += secsSlept;
                    // if(secsNotStalled > 0 && secsNotStalled % 100 == 0){
                    //     info("Not stalled, did %ld phases since last check", numPhases-lastNumPhases);
                    // }
        ...
        if (secsStalled > zinfo->deadlock_killing_cycles) {
