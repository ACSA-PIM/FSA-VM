...
#include <vector>
/*-------------legacy paging------------*/
NormalPaging::NormalPaging(PagingStyle select)
    : mode(select), cur_page_table_num(0) {
...
Address NormalPaging::access(MemReq &req) {
    ...
    if (mode == Legacy_Normal) {
        ...
        req.cycle += (zinfo->mem_access_time *
                      2); // assume memory accessing cycle is 117 cycles
        if (!ptr)
            return PAGE_FAULT_SIG;
    } else if (mode == Legacy_Huge) {
...
/*
 *@function: remove page directory, although this is seldom used , but it's
 *provided for complete function
 */
void NormalPaging::remove_root_directory() { cur_page_table_num = 0; }

/*
 *@function: remove page table whose address is recorded in page directory's
 *page_table_idth entry
 *@param page_table_id: entry num of page directory,page table this entry
 *pointed to can be removed if the whole page table is unuseful
 */
 ...
 *@param pd_entry: list which record entry num of page direcotry , these entries
 *point to page table will be removed
 */
...
bool NormalPaging::allocate_page_table(Address addr, Address size) {
    ...
    for (unsigned i = 0; i < page_num; i++) {
        if (!allocate_one_pagetable(base_entry_id + i, allocate_time)) {
            debug_printf("allocate page table for %d th entry failed",
                         base_entry_id + i);
            succeed = false;
        }
    }
...
bool NormalPaging::remove_page_table(Address addr, Address size) {
    ...
    for (unsigned i = 0; i < page_num; i++) {
        if (!(remove_page_table(base_pageNo + page_num))) {
            debug_printf(
                "remove page table of %d th entry of page directory failed",
                base_pageNo + page_num);
            succeed = false;
        }
    }
    return succeed;
...
/*--------------PAE Paging------------*/
PAEPaging::PAEPaging(PagingStyle select)
    : mode(select), cur_pdt_num(0), cur_pt_num(0) {
...
int PAEPaging::map_page_table(Address addr, Page *pg_ptr,
    ...
    if (mode == PAE_Normal) {
        // page table offset
        unsigned pt_id = get_pagetable_off(addr, mode);
        if ((table = allocate_page_table(pdp_id, pd_id, allocate_time)) ==
            NULL) {
...
bool PAEPaging::unmap_page_table(Address addr) {
    ...
    unsigned pd_id = get_page_directory_off(addr, mode);
    PageTable *pd_ptr =
        get_next_level_address<PageTable>(page_directory_pointer, pdp_id);
    if (pd_ptr) {
        if (mode == PAE_Normal) {
            PageTable *pt_ptr =
                get_next_level_address<PageTable>(pd_ptr, pd_id);
            if (pt_ptr) {
                unsigned pt_id = get_pagetable_off(addr, mode);
...
PageTable *PAEPaging::allocate_page_table(unsigned pdpt_entry_id,
    ...
    // page directory
    PageTable *table = get_next_level_address<PageTable>(page_directory_pointer,
                                                         pdpt_entry_id);
    PageTable *page = NULL;
 ...
 *%attention: information of current page tables must can be contained within
 *page directory entries before or when allocating page table
 *@param pdt_entry: list of (page directory pointer entry id , page direcotry
 *entry id)
 */
bool PAEPaging::allocate_page_table(pair_list pdt_entry) {
    bool succeed = true;
    int time = 0;
    for (pair_list_ptr it = pdt_entry.begin(); it != pdt_entry.end(); it++) {
        if (!(allocate_page_table((*it).first, (*it).second), time)) {
            succeed = false;
            debug_printf("allocate page directory for entry %d for page "
                         "directory pointer failed");
        }
    }
    return succeed;
}
...
PageTable *PAEPaging::allocate_pdt(unsigned pdpt_entry, int &alloc_time) {
    ...
    // pdpt_entry of page_direcotry_pointer has already pointed to page
    // directory
    if ((page_dir = get_next_level_address<PageTable>(page_directory_pointer,
                                                      pdpt_entry)) != NULL)
        return page_dir;
    else {
        // page directory
...
bool PAEPaging::allocate_pdt(entry_list pdpt_entry) {
    ...
    for (entry_list_ptr it = pdpt_entry.begin(); it != pdpt_entry.end(); it++) {
        if (!(allocate_pdt(*it, time))) {
            succeed = false;
            debug_printf("allocate page directory for %d entry of page "
                         "directory pointer failed",
                         *it);
        }
    }
...
bool PAEPaging::allocate_page_table(Address addr, Address size) {
    ...
    for (unsigned i = 0; i < page_num; i++) {
        if (allocate_page_table(pdp_offset, i + pd_offset, time) == NULL) {
            debug_printf("allocate %d page table for 0x%x failed", page_num,
                         addr);
            succeed = false;
        }
    }
...
bool PAEPaging::allocate_pdt(Address addr, Address size) {
    ...
    int time;
    if (addr & 0x7fffffff) {
        fatal("base address must align with 1GB when allocate page directory "
              "in PAE mode");
        succeed = false;
    }
    ...
    for (unsigned i = 0; i < page_dir_num; i++) {
        if (allocate_pdt(base_addr + i, time) == NULL) {
            debug_printf(
                "allocate number of %d page directory for address 0x%x failed",
                page_dir_num, addr);
            succeed = false;
        }
    }
    ...
    for (unsigned i = 0; i < ENTRY_4; i++) {
        if (is_present(page_directory_pointer, i)) {
            if (remove_pdt(i) == false) {
                debug_printf("remove page directory entry %d of page direcotry "
                             "pointer pointed to failed",
                             i);
            }
        }
    }
...
bool PAEPaging::remove_pdt(Address addr, Address size) {
    ...
    for (unsigned i = 0; i < dir_num; i++) {
        if (remove_pdt(page_dir_ptr + i)) {
            debug_printf(
                "remove page directory whose arrange is (%ld,%ld) failed", addr,
                addr + size);
            succeed = false;
        }
    }
...
bool PAEPaging::remove_pdt(unsigned pdp_entry_id) {
    bool succeed = false;
    PageTable *page_dir =
        get_next_level_address<PageTable>(page_directory_pointer, pdp_entry_id);
    if (page_dir) {
        if (mode == PAE_Huge) {
            ...
            for (i = 0; i < ENTRY_512; i++) {
                if (is_present(page_dir, i)) {
                    if (remove_page_table(pdp_entry_id, i) == false) {
                        debug_printf("remove %d entry of page table failed!",
                                     i);
                        succeed = false;
                    }
                }
        ...
        debug_printf("entry %d of page direcotry pointer pointed to nothing",
                     pdp_entry_id);
    }
    return succeed;
}
...
bool PAEPaging::remove_pdt(entry_list pdpt_entry) {
    ...
    for (entry_list_ptr it = pdpt_entry.begin(); it != pdpt_entry.end(); it++) {
        if (remove_pdt(*it) == false) {
            debug_printf("remove page directory pointed by entry %d of page "
                         "directory pointer failed",
                         *it);
            succeed = false;
        }
...
bool PAEPaging::remove_page_table(Address addr, Address size) {
    ...
    if (addr & 0x1fffff) {
        ...
        unsigned page_dir_ptr =
            get_page_directory_pointer_off(addr, PAE_Normal);
        ...
        for (unsigned i = 0; i < page_table_num; i++) {
            if (remove_page_table(page_dir_ptr, page_dir + i) == false) {
                succeed = false;
                debug_printf("remove (page directory pointer,page "
                             "directory)-(%d,%d) pointed page table failed",
                             page_dir_ptr, page_dir + i);
            }
        }
    }
    return succeed;
 ...
 *@param pdp_entry: list of (page_directory_pointer entry id , page_directory
 *entry id)
...
bool PAEPaging::remove_page_table(pair_list pdp_entry) {
    ...
    for (pair_list_ptr it = pdp_entry.begin(); it != pdp_entry.end(); it++) {
        if (remove_page_table((*it).first, (*it).second) == false) {
            debug_printf("remove page table for (page direcotor pointer entry "
                         "id: %d , page directory id:%d) failed",
                         (*it).first, (*it).second);
            succeed = false;
        }
    }
...
/*
 *@function: delete page table referred by (pdpt_num[entry id of page directory
 *pointer],pd_num[entry id of page directory])
...
bool PAEPaging::remove_page_table(unsigned pdpt_num, unsigned pd_num) {
    ...
    PageTable *page_dir =
        get_next_level_address<PageTable>(page_directory_pointer, pdpt_num);
    if (page_dir) {
...
LongModePaging::LongModePaging(PagingStyle select)
...
LongModePaging::~LongModePaging() { remove_root_directory(); }
...
int LongModePaging::map_page_table(uint32_t req_id, Address addr, Page *pg_ptr,
                                   bool is_write) {
    BasePDTEntry *entry;
...
int LongModePaging::map_page_table(Address addr, Page *pg_ptr,
    ...
    if (mode == LongMode_Normal) {
        ...
        if (!table) {
            // debug_printf("allocate page directory for LongMode_Middle
            // failed!");
        ...
        if (!table) {
            // debug_printf("allocate page directory pointer for LongMode_Huge
            // failed!");
...
inline PageTable *
LongModePaging::get_tables(unsigned level,
...
uint64_t LongModePaging::loadPageTable(MemReq &req, uint64_t startCycle,
...
uint64_t LongModePaging::loadPageTables(MemReq &req,
...
Address LongModePaging::access(MemReq &req, g_vector<MemObject *> &parents,
                               g_vector<uint32_t> &parentRTTs,
                               BaseCoreRecorder *cRec, bool sendPTW) {
    g_vector<uint64_t> pgt_addrs;
    ...
    uint64_t lineAddr = pgAddr >> lineBits;
    MESIState dummyState = MESIState::I;
    MemReq pwc_req = {lineAddr,    GETS,       req.childId,
                        &dummyState, req.cycle, req.childLock,
                        dummyState,  req.srcId,  req.flags};
    ...
    if (mode == LongMode_Middle) {
        ...
        assert(pd_id != (unsigned)(-1));
        assert(pt_id != (unsigned)(-1));
        // pgAddr = getPGTAddr(((PageTable *)ptr)->get_page_no(), pd_id);
        // pwc_hit = pw_cache->access(pgAddr, 2, req);
        // if(pwc_hit == false) pgt_addrs.push_back(pgAddr);
        // point to page table
        ptr = get_next_level_address<void>((PageTable *)ptr, pd_id);
        if (!ptr) {
        ...
        pgt_addrs.push_back(
            getPGTAddr(((PageTable *)ptr)->get_page_no(), pt_id));
        // point to page
    ...
    if (req.triggerPageShared && pdt_ptr->remapped_times)
...
// allocate
PageTable *
LongModePaging::allocate_page_directory_pointer(unsigned pml4_entry_id,
                                                int &allocate_time) {
    // allocate_time = 0;
...
PageTable *LongModePaging::allocate_page_directory(unsigned pml4_entry_id,
...
bool LongModePaging::remove_page_directory(unsigned pml4_entry_id,
...
bool LongModePaging::remove_page_table(unsigned pml4_entry_id,
                                       unsigned pdp_entry_id,
                                       unsigned pd_entry_id) {
    assert(mode == LongMode_Normal);
    PageTable *pdp_table = NULL;
