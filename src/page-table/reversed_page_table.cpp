...
int ReversedPaging::map_page_table(uint32_t req_id, Address addr, Page *pg_ptr,
                                   bool is_write) {
...
uint64_t ReversedPaging::remap_page_table(Address ppn, Address dst_ppn) {
    ...
    if (reversed_pgt.count(ppn)) {
        // std::cout<<"remap:"<<ppn<<" -> "<<dst_ppn<<"
        // vpn:"<<get_vpn(ppn)<<std::endl;
        latency += zinfo->mem_access_time;
        reversed_pgt[dst_ppn] = reversed_pgt[ppn];
        (reversed_pgt[dst_ppn])
            ->get_pgt_entry()
            ->set_next_level_address(dst_ptr);
    ...
    // {
    // 	//std::cout<<"remap:"<<dst_ppn<<"("<<ppn<<")"<<" ->
    // "<<dst_ppn<<"("<<dst_dram<<")"<<" vpn:"<<get_vpn(dst_ppn)<<std::endl;
    // 	latency += zinfo->mem_access_time;
...
void ReversedPaging::calculate_stats() { paging->calculate_stats(); }

void ReversedPaging::calculate_stats(std::ofstream &vmof) {
    long unsigned page_num = (long unsigned)reversed_pgt.size();
    vmof << "Total pages:" << page_num
         << ", remapped pages:" << remapped_page_num << std::endl;
