...
class NormalPaging : public BasePaging {
  public:
    ...
    virtual int map_page_table(uint32_t req_id, Address addr, Page *pg_ptr,
                               bool is_write) {
        return map_page_table(addr, pg_ptr);
    }
    ...
    unsigned get_page_table_num() { return cur_page_table_num; }

    virtual PageTable *get_root_directory() { return page_directory; }
    ...
    virtual void calculate_stats() {
        ...
        info("overhead of extended storage:%f MB",
    ...
    virtual void calculate_stats(std::ofstream &vmof) {
        ...
        vmof << "overhead of extended storage:"
...
class PAEPaging : public BasePaging {
  public:
    ...
    unsigned get_pdt_num() { return cur_pdt_num; }

    virtual PageTable *get_root_directory() { return page_directory_pointer; }
    ...
    virtual void calculate_stats() {
        long unsigned overhead =
            (long unsigned)(cur_pt_num + cur_pdt_num) * PAGE_SIZE;
...
class LongModePaging : public BasePaging {
  public:
    ...
    virtual PagingStyle get_paging_style() { return mode; }
    virtual void setPTW(BasePageTableWalker* _ptw) {ptw = _ptw; pwc = ptw->Getpwc();}
