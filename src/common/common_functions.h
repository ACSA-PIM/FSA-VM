...
{
	#ifdef DEBUG_PRINT
		format_str ="ZSIM-Ramulator Debug:" + format_str+"\n";
		va_list parg;
...
{
	format_str ="ZSIM-Ramulator Warning:" + format_str;
	va_list parg;
	va_start(parg , format_str);
	vfprintf(stdout , format_str.c_str() , parg);
	va_end(parg);
...
{
	format_str ="ZSIM-Ramulator Fatal:" + format_str;
	va_list parg;
	va_start(parg , format_str);
	vfprintf(stdout , format_str.c_str() , parg);
	va_end(parg);
...
inline PagingStyle string_to_pagingmode( const char* mode_str)
{
	...
	if( !strcmp(mode_str , "Hash_Normal") )
		return Hash_Normal;
	if( !strcmp(mode_str , "Cuckoo_Hash") )
		return Cuckoo_Hash;
...
inline uint64_t get_page_size_by_mode( PagingStyle mode)
{
	...
	if( mode == Legacy_Normal || mode==PAE_Normal
			|| mode==LongMode_Normal || mode==Hash_Normal || mode==Cuckoo_Hash)
		return (4*kb);	//4KB
	if (mode==Legacy_Huge)
		return (4*mb);	//4MB
...
inline MemType memStrToType(std::string type_str)
{
	if(type_str == "Simple")
		return MEM_SIMPLE;
	if(type_str == "MD1")
		return MEM_MD1;
	if(type_str == "WeaveMD1")
		return MEM_WEAVEMD1;
	if(type_str == "WeaveSimple")
		return MEM_WEAVESIMPLE;
	if(type_str == "DDR")
		return MEM_DDR;
	if(type_str == "DRAMSim")
		return MEM_DRAMSIM;
	if(type_str == "Detailed")
		return MEM_DETAILED;
	if(type_str == "Ramulator")
		return MEM_RAMULATOR;
}
