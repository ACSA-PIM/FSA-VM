...
KNOB<string> KnobProcConfigFile(KNOB_MODE_WRITEONCE, "pintool",
        "procConfig", "", "process config file name (only needed for the first simulated process)");

KNOB<INT32> KnobHarnessPid(KNOB_MODE_WRITEONCE, "pintool",
        "harnessPid", "", "Harness process id");

KNOB<string> KnobProfileFile(KNOB_MODE_WRITEONCE, "pintool",
...
static inline void setCid(uint32_t tid, uint32_t cid) {
    ...
    // std::cout<<"set core "<<cid<<" with proc "<< procIdx <<" thread "<< tid <<std::endl;
    if(cores[tid]->GetProcIdx() != procIdx){
        cores[tid]->flushTlb();
    }
	cores[tid]->SetProcIdx(procIdx);
    cores[tid]->SetThreadIdx(tid);
    cores[tid]->SetThreadBBLStatus(&(zinfo->sched->getBBLStatus(procIdx, tid)));
	//initialize paging
...
void EnterOffloadRegion();
void ExitOffloadRegion();
...
VOID JoinAndLoadSingle(THREADID tid, ADDRINT addr, UINT32 size, BOOL inOffloadRegion) {
    Join(tid);
    fPtrs[tid].loadPtr(tid, addr, size, inOffloadRegion);
}

VOID JoinAndStoreSingle(THREADID tid, ADDRINT addr, UINT32 size, BOOL inOffloadRegion) {
    Join(tid);
    fPtrs[tid].storePtr(tid, addr, size, inOffloadRegion);
}

VOID JoinAndBasicBlock(THREADID tid, ADDRINT bblAddr, BblInfo* bblInfo, BOOL inOffloadRegion) {
    Join(tid);
    fPtrs[tid].bblPtr(tid, bblAddr, bblInfo, inOffloadRegion);
}

VOID JoinAndRecordBranch(THREADID tid, ADDRINT branchPc, BOOL taken, ADDRINT takenNpc, ADDRINT notTakenNpc, BOOL inOffloadRegion) {
    Join(tid);
    fPtrs[tid].branchPtr(tid, branchPc, taken, takenNpc, notTakenNpc, inOffloadRegion);
}

VOID JoinAndPredLoadSingle(THREADID tid, ADDRINT addr, BOOL pred, UINT32 size, BOOL inOffloadRegion) {
    Join(tid);
    fPtrs[tid].predLoadPtr(tid, addr, pred, size, inOffloadRegion);
}

VOID JoinAndPredStoreSingle(THREADID tid, ADDRINT addr, BOOL pred, UINT32 size, BOOL inOffloadRegion) {
    Join(tid);
    fPtrs[tid].predStorePtr(tid, addr, pred, size, inOffloadRegion);
}
...
VOID NOPLoadStoreSingle(THREADID tid, ADDRINT addr, UINT32 size, BOOL inOffloadRegion) {}
VOID NOPBasicBlock(THREADID tid, ADDRINT bblAddr, BblInfo* bblInfo, BOOL inOffloadRegion) {}
VOID NOPRecordBranch(THREADID tid, ADDRINT addr, BOOL taken, ADDRINT takenNpc, ADDRINT notTakenNpc, BOOL inOffloadRegion) {}
VOID NOPPredLoadStoreSingle(THREADID tid, ADDRINT addr, BOOL pred, UINT32 size, BOOL inOffloadRegion) {}
...
VOID FFBasicBlock(THREADID tid, ADDRINT bblAddr, BblInfo* bblInfo, BOOL inOffloadRegion) {
...
VOID FFIBasicBlock(THREADID tid, ADDRINT bblAddr, BblInfo* bblInfo, BOOL inOffloadRegion) {
...
VOID FFIEntryBasicBlock(THREADID tid, ADDRINT bblAddr, BblInfo* bblInfo, BOOL inOffloadRegion) {
    ffiInstrsDone += *ffiFFStartInstrs - *ffiPrevFFStartInstrs; //add all instructions executed in the NFF phase
    FFIAdvance();
    assert(ffiNFF);
    ffiNFF = false;
    fPtrs[tid] = GetFFPtrs();
    FFIBasicBlock(tid, bblAddr, bblInfo, inOffloadRegion);
}
...
void EnterOffloadRegion() {
    ...
    procTreeNode->enterOffloadRegion();
    __sync_synchronize();
}

void ExitOffloadRegion() {
    assert(procTreeNode->isInOffloadRegion());
    procTreeNode->exitOffloadRegion();
    __sync_synchronize();
}
        ...
        if(zinfo->enable_pim_mode){
            if(!zinfo->maxOffloadInstrsIssued){
                if (totalOffloadInstrs >= zinfo->maxOffloadInstrs){
                    zinfo->maxOffloadInstrsIssued = true;
                    info("Max offload instructions issued (%ld)", totalOffloadInstrs);
                }
            }
            uint64_t totalPendingEvents = 0;
            for (int i = 0; i < zinfo->numMemoryControllers; i++){
                totalPendingEvents += zinfo->memoryControllers[i]->getPendingEvents();
            }
            if(zinfo->maxOffloadInstrsIssued && totalPendingEvents == 0){
                zinfo->terminationConditionMet = true;
...
VOID EndOfPhaseActions() {
    ...
    // info("Simulation entering bound phase");
}
...
VOID Instruction(INS ins, bool inOffloadRegion) {
    ...
    if (!procTreeNode->isInFastForward() || !zinfo->ffReinstrument) {
        ...
        if (INS_IsMemoryRead(ins)) {
            if (!INS_IsPredicated(ins)) {
                INS_InsertCall(ins, IPOINT_BEFORE, LoadFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_MEMORYREAD_EA, IARG_MEMORYREAD_SIZE /*LOIS: track the size of the read*/,  IARG_BOOL, inOffloadRegion, IARG_END);
            } else {
                INS_InsertCall(ins, IPOINT_BEFORE, PredLoadFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_MEMORYREAD_EA, IARG_EXECUTING, IARG_MEMORYREAD_SIZE /*LOIS: track the size of the read*/, IARG_BOOL, inOffloadRegion, IARG_END);
            }
        }

        if (INS_HasMemoryRead2(ins)) {
            if (!INS_IsPredicated(ins)) {
                INS_InsertCall(ins, IPOINT_BEFORE, LoadFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_MEMORYREAD2_EA, IARG_MEMORYREAD_SIZE /*LOIS: track the size of the read*/,  IARG_BOOL, inOffloadRegion, IARG_END);
            } else {
                INS_InsertCall(ins, IPOINT_BEFORE, PredLoadFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_MEMORYREAD2_EA, IARG_EXECUTING, IARG_MEMORYREAD_SIZE /*LOIS: track the size of the read*/, IARG_BOOL, inOffloadRegion, IARG_END);
            }
        }

        if (INS_IsMemoryWrite(ins)) {
            if (!INS_IsPredicated(ins)) {
                INS_InsertCall(ins, IPOINT_BEFORE,  StoreFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_MEMORYWRITE_EA, IARG_MEMORYREAD_SIZE /*LOIS: track the size of the read*/, IARG_BOOL, inOffloadRegion, IARG_END);
            } else {
                INS_InsertCall(ins, IPOINT_BEFORE,  PredStoreFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_MEMORYWRITE_EA, IARG_EXECUTING, IARG_MEMORYREAD_SIZE /*LOIS: track the size of the read*/, IARG_BOOL, inOffloadRegion, IARG_END);
            }
        }
        ...
        if (INS_Category(ins) == XED_CATEGORY_COND_BR) {
            INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR) IndirectRecordBranch, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                    IARG_INST_PTR, IARG_BRANCH_TAKEN, IARG_BRANCH_TARGET_ADDR, IARG_FALLTHROUGH_ADDR, IARG_BOOL, inOffloadRegion, IARG_END);
        }
...
VOID Trace(TRACE trace, VOID *v) {
    if (!procTreeNode->isInFastForward() || !zinfo->ffReinstrument) {
        ...
        for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
            ...
            bool inOffloadRegion = false;
            if(procTreeNode->isInOffloadRegion()){
                inOffloadRegion = true;
            }
            BBL_InsertCall(bbl, IPOINT_BEFORE /*could do IPOINT_ANYWHERE if we redid load and store simulation in OOO*/, (AFUNPTR)IndirectBasicBlock, IARG_FAST_ANALYSIS_CALL,
                 IARG_THREAD_ID, IARG_ADDRINT, BBL_Address(bbl), IARG_PTR, bblInfo, IARG_BOOL, inOffloadRegion, IARG_END);
        }
    }
    ...
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
        ...
        for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins)) {
            Instruction(ins,inOffloadRegion);
        }
    }
}
...
//#####*************find, allocate and remap shared memory*******#####//
/*
 *@function: parse [maps file] to get all [linear virtual address region]
 *@params:
  libs_to_region: return object, mappings between [library name] 
		...
		const char* file_name )
{
	...
	unsigned long tmp_start = 0x0;
	unsigned long tmp_end = 0x0;
	char *dash;
	g_string lib_str;
	if(fp){
		while( fgets(buf, 128, fp)){
			//find all share library memory region
			if( strstr(buf, ".so") && strstr(buf, "lib")&& !strstr(buf,"w")){
				char* lib_ptr = strchr(buf,'/');
				dash = strstr(buf, "-");
				if( dash ){
					lib_str = lib_ptr;
					*dash = '\0';
					//get start and end virtual address of continuous shared memory region
...
/*
 *@function: get memory region information of shared 
			 memory by parsing /proc/pid/maps
 *@params: 
		tid: process id of physical machine
	    procIdx: process id transformed by zsim 
		corresponding to tid specified process
 */
void GetSharedMemory( THREADID tid, THREADID procIdx )
{
	...
	//remap virtual shared linear regions 
	//of process to physical shared memory region
}

void SimThreadStart(THREADID tid) {
    info("Thread %d starting", tid);
    if (tid > MAX_THREADS) panic("tid > MAX_THREADS");
    //get shared memory of every process
...
VOID SimEnd() {
    ...
    if (procIdx == 0) {
        ...
        if(zinfo->ramulatorWrapper) {
            zinfo->ramulatorWrapper->finish();
            zinfo->ramulatorWrapper->printall();
        }
        for (auto mem : zinfo->memoryControllers) mem->dump();

        std::ofstream vmof;
        std::string vm_outfile = zinfo->outputDir;
        vm_outfile += "/virtual_memory.out";
        vmof.open(vm_outfile,std::ios_base::out);
    ...
    switch (op) {
        ...
        case ZSIM_MAGIC_OP_PIM_MP_BEGIN:
            if (!zinfo->ignoreHooks) {
                futex_lock(&zinfo->ffLock);
                if (!procTreeNode->isInOffloadRegion()) {
                    info("[OFFLOAD_MP_BEGIN], entering offload MP region");
                    EnterOffloadRegion();
                    assert(procTreeNode->isInOffloadRegion());
                } else {
                    warn("Ignoring OFFLOAD_MP_BEGIN magic op, already in offload region");
                }
                futex_unlock(&zinfo->ffLock);
            }
            return;
        case ZSIM_MAGIC_OP_PIM_MP_END:
            if (!zinfo->ignoreHooks) {
                futex_lock(&zinfo->ffLock);
                if (procTreeNode->isInOffloadRegion()) {
                    info("[OFFLOAD_MP_END], exiting offload region");
                    ExitOffloadRegion();
                    assert(!procTreeNode->isInOffloadRegion());
                } else {
                    warn("Ignoring OFFLOAD_MP_END magic op, not in offload region");
                }
                futex_unlock(&zinfo->ffLock);
            }
            return;
...
static EXCEPT_HANDLING_RESULT InternalExceptionHandler(THREADID tid, EXCEPTION_INFO *pExceptInfo, PHYSICAL_CONTEXT *pPhysCtxt, VOID *) {
    ...
    if (PIN_GetFaultyAccessAddress(pExceptInfo, &faultyAccessAddr)) {
        ...
        fprintf(stderr, "%s[%d]  Caused by invalid %s access to address 0x%lx\n", logHeader, tid, faultyAccessStr, faultyAccessAddr);
    }
